...
//     // Helper function to get campaign data from the campaigns mapping
//     function getCampaignData(uint32 campaignId) internal view returns (
//         address campaignCreator,
//         address tokenAddress,
//         address uniswapPair,
//         uint128 targetAmount,
//         uint128 amountRaised,
//         uint64 campaignDeadline,
//         uint32 reserveRatio,
...
//         string memory tokenFileId,
//         string memory whitepaperFileId
//     ) {
//         IERC20 token;
//         (
//             campaignCreator,
...
//             tokenFileId,
//             whitepaperFileId
//         ) = launchpad.campaigns(campaignId);
...
//         // Deploy Launchpad
//         launchpad = new Launchpad(owner, address(usdc), address(router), address(factory), PROMOTION_FEE);
...
//         // Verify campaign info by accessing campaigns mapping
//         (
//             address campaignCreator,
//             address tokenAddress,
//             ,  // uniswapPair
...
//             string memory campaignSymbol,
//             string memory campaignDescription,
//             string memory campaignTokenFileId,
//             string memory campaignWhitepaperFileId
//         ) = getCampaignData(campaignId);

//         assertEq(id, 1, "Campaign ID mismatch");
//         assertEq(campaignCreator, creator, "Creator mismatch");
//         assertEq(campaignName, name, "Name mismatch");
//         assertEq(campaignSymbol, symbol, "Symbol mismatch");
//         assertEq(campaignDescription, description, "Description mismatch");
//         assertEq(campaignTokenFileId, tokenFileId, "Token file ID mismatch");
//         assertEq(campaignWhitepaperFileId, "", "Whitepaper file ID should be empty");
//         assertEq(targetAmount, targetFunding, "Target amount mismatch");
//         assertEq(totalSupplyValue, totalSupply, "Total supply mismatch");
//         assertEq(reserveRatioValue, reserveRatio, "Reserve ratio mismatch");
//         assertEq(campaignDeadline, deadline, "Deadline mismatch");
//         assertTrue(isActive, "Campaign should be active");
//         assertFalse(isFundingComplete, "Campaign should not be funded yet");
//         assertFalse(isCancelled, "Campaign should not be cancelled");
//         assertFalse(isPromoted, "Campaign should not be promoted");
//         assertFalse(isDAOEnabled, "DAO should not be enabled");
...
//         assertEq(tokensForSale, expectedTokensForSale, "Tokens for sale mismatch");
//         assertEq(creatorAllocation, expectedCreatorAllocation, "Creator allocation mismatch");
//         assertEq(liquidityAllocation, expectedLiquidityAllocation, "Liquidity allocation mismatch");
//         assertEq(platformFeeTokens, expectedPlatformFee, "Platform fee mismatch");
...
//         (,,,,,,,,,,,bool _isDAOEnabled,,,,,,,,, string memory _tokenFileId, string memory _whitepaperFileId) = getCampaignData(campaignId);
//         assertEq(_tokenFileId, tokenFileId, "Token file ID should match Hedera file ID");
//         assertEq(_whitepaperFileId, whitepaperFileId, "Whitepaper file ID should match");
//         assertTrue(_isDAOEnabled, "DAO should be enabled");
...
//         (,,,,,,,,,,,,,,,,,,,, string memory name1, string memory symbol1,, string memory tokenFileId1, string memory whitepaperFileId1) = getCampaignData(campaignId1);
//         bool isDAOEnabled1;
//         (,,,,,,,,,,,isDAOEnabled1,,,,,,,,,,,,,,) = getCampaignData(campaignId1);

//         // Get second campaign data
//         (,,,,,,,,,,,,,,,,,,,, string memory name2, string memory symbol2,, string memory tokenFileId2, string memory whitepaperFileId2) = getCampaignData(campaignId2);
//         bool isDAOEnabled2;
//         (,,,,,,,,,,,isDAOEnabled2,,,,,,,,,,,,,,) = getCampaignData(campaignId2);

//         assertEq(name1, "Token One", "First campaign name mismatch");
//         assertEq(name2, "Token Two", "Second campaign name mismatch");
//         assertEq(tokenFileId1, "0.0.111111", "First campaign file ID mismatch");
//         assertEq(tokenFileId2, "0.0.222222", "Second campaign file ID mismatch");
//         assertEq(whitepaperFileId1, "", "First campaign should have no whitepaper");
//         assertEq(whitepaperFileId2, "0.0.333333", "Second campaign whitepaper mismatch");
//         assertFalse(isDAOEnabled1, "First campaign should not have DAO enabled");
//         assertTrue(isDAOEnabled2, "Second campaign should have DAO enabled");
...
//         (address creator,address tokenAddress,address uniswapPair,uint128 targetAmount,uint128 amountRaised,uint64 deadline,uint32 reserveRatio,uint32 blockNumberCreated,bool isActive,bool isFundingComplete,bool isCancelled,bool isPromoted,bool isDAOEnabled,uint128 tokensSold,uint128 totalSupply,uint128 tokensForSale,uint128 creatorAllocation,uint128 liquidityAllocation,uint128 platformFeeTokens,uint128 promotionalOgPoints,uint32 id,string memory name,string memory symbol,string memory description,string memory tokenFileId,string memory whitepaperFileId) = getCampaignData(campaignId);

//         // Verify token was deployed
//         assertTrue(tokenAddress != address(0), "Token address should not be zero");

//         // Verify token properties
//         TokenFacet token = TokenFacet(tokenAddress);
...
//         // Verify promotion
//         (address creator,address tokenAddress,address uniswapPair,uint128 targetAmount,uint128 amountRaised,uint64 deadline,uint32 reserveRatio,uint32 blockNumberCreated,bool isActive,bool isFundingComplete,bool isCancelled,bool isPromoted,bool isDAOEnabled,uint128 tokensSold,uint128 totalSupply,uint128 tokensForSale,uint128 creatorAllocation,uint128 liquidityAllocation,uint128 platformFeeTokens,uint128 promotionalOgPoints,uint32 id,string memory name,string memory symbol,string memory description,string memory tokenFileId,string memory whitepaperFileId) = getCampaignData(campaignId);
//         assertTrue(isPromoted, "Campaign should be promoted");
//         assertEq(promotionalOgPoints, 1000, "Promotional OG points should be set");
...
//         // Verify purchase
//         (address creator,address tokenAddress,address uniswapPair,uint128 targetAmount,uint128 amountRaised,uint64 deadline,uint32 reserveRatio,uint32 blockNumberCreated,bool isActive,bool isFundingComplete,bool isCancelled,bool isPromoted,bool isDAOEnabled,uint128 tokensSold,uint128 totalSupply,uint128 tokensForSale,uint128 creatorAllocation,uint128 liquidityAllocation,uint128 platformFeeTokens,uint128 promotionalOgPoints,uint32 id,string memory name,string memory symbol,string memory description,string memory tokenFileId,string memory whitepaperFileId) = getCampaignData(campaignId);
//         assertEq(amountRaised, purchaseAmount, "Amount raised should match purchase");
//         assertGt(tokensSold, 0, "Tokens should be sold");
...
//         assertLt(promotionalOgPoints, 1000, "Some OG points should have been distributed");
...
//         (address creator,address tokenAddress,address uniswapPair,uint128 targetAmount,uint128 amountRaised,uint64 deadline,uint32 reserveRatio,uint32 blockNumberCreated,bool isActive,bool isFundingComplete,bool isCancelled,bool isPromoted,bool isDAOEnabled,uint128 tokensSold,uint128 totalSupply,uint128 tokensForSale,uint128 creatorAllocation,uint128 liquidityAllocation,uint128 platformFeeTokens,uint128 promotionalOgPoints,uint32 id,string memory name,string memory symbol,string memory description,string memory tokenFileId,string memory whitepaperFileId) = getCampaignData(campaignId);
//         assertFalse(isActive, "Campaign should not be active");
//         assertTrue(isCancelled, "Campaign should be cancelled");
...
//         (address creator,address tokenAddress,address uniswapPair,uint128 targetAmount,uint128 amountRaised,uint64 deadline,uint32 reserveRatio,uint32 blockNumberCreated,bool isActive,bool isFundingComplete,bool isCancelled,bool isPromoted,bool isDAOEnabled,uint128 tokensSold,uint128 totalSupply,uint128 tokensForSale,uint128 creatorAllocation,uint128 liquidityAllocation,uint128 platformFeeTokens,uint128 promotionalOgPoints,uint32 id,string memory name,string memory symbol,string memory description,string memory tokenFileId,string memory whitepaperFileId) = getCampaignData(campaignId);
//         assertTrue(isFundingComplete, "Funding should be complete");
//         assertFalse(isActive, "Campaign should not be active");
...
//             assertEq(tokensSold, tokensForSale, "All tokens should be sold");
//         }
//     }
...
//         assertEq(creatorTokenBalance, creatorAllocation, "Creator should receive allocation");
//     }
...
//         assertGe(launchpadTokenBalance, platformFeeTokens, "Platform should receive fee tokens");
//     }
...
//         (address _creator,,, uint128 _targetAmount,, uint64 _deadline, uint32 _reserveRatio,, bool _isActive, bool _isFundingComplete, bool _isCancelled,, bool _isDAOEnabled,, uint128 _totalSupply,,,,,, uint32 id, string memory _name, string memory _symbol, string memory _description, string memory _tokenFileId, string memory _whitepaperFileId) = getCampaignData(campaignId);
...
//         assertEq(_tokenFileId, tokenFileId);
//         assertEq(_whitepaperFileId, whitepaperFileId);
//         assertEq(_targetAmount, targetFunding);
//         assertEq(_totalSupply, totalSupply);
//         assertEq(_reserveRatio, reserveRatio);
//         assertEq(_deadline, deadline);
//         assertTrue(_isDAOEnabled);
//         assertTrue(_isActive);
//         assertFalse(_isFundingComplete);
//         assertFalse(_isCancelled);
